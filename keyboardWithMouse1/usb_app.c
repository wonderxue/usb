...
void getDeviceDescriptor()
{
    ...
    pdevice->bMaxPacketSize0 = app.endpoint0->maxPacketSize;
...
void getConfigurationDescriptor()
{
    ...
    pconfig->wTotalLength = sizeof(USB_ConfigurationDescriptor) + sizeof(USB_InterfaceDescriptor) + sizeof(USB_HIDDescriptor) + 2*sizeof(USB_EndpointDescriptor);
    ...
    pendpoint[0].wMaxPacketSize = app.endpoint1->maxPacketSize;
...
char sendReport(unsigned char *buff,int length)
{
    app.pwrite=writeBuffer;
    //printf("asdasd\n");
