...
static unsigned char readBuffer[READ_MAX_BUFFER_SIZE];
static unsigned char writeBuffer[WRITE_MAX_BUFFER_SIZE];
extern void usbWritePMA(unsigned short addr, const void *buffer, int length);
extern void usbInit();
extern unsigned char ReportDescriptor_KEYBOARD[REPORTDESCRIPTOR_SIZE_KEYBOARD];
extern unsigned char ReportDescriptor_MOUSE[REPORTDESCRIPTOR_SIZE_MOUSE];
extern USB_App app;
...
void getDeviceDescriptor()
{
    ...
    pdevice->bMaxPacketSize0 = app.endpoint0->maxPacketSizeIn;
    pdevice->idVendor = 0x1022;  // STMicroelectronics (http://www.linux-usb.org/usb.ids)
    pdevice->idProduct = 0x1125; // STM microSD Flash pdevice
    pdevice->bcdDevice = 0x0012;
...
void getConfigurationDescriptor()
{
    ...
    pconfig->wTotalLength = sizeof(USB_ConfigurationDescriptor) + 2 * sizeof(USB_InterfaceDescriptor) + 2 * sizeof(USB_HIDDescriptor) + 3 * sizeof(USB_EndpointDescriptor);
    pconfig->bNumInterfaces = 2;      // 接口数
    pconfig->bConfigurationValue = 1; // 此配置的编号
    pconfig->iConfiguration = 4;      // 配置名字符串序号(0表示没有)
    ...
    pinterface->bInterfaceNumber = 0;      // 此接口的编号
    pinterface->bAlternateSetting = 0;     // 可用的备用接口编号
    pinterface->bNumEndpoints = 2;         // 除了端点0外, 此接口还需要的端点数
    pinterface->bInterfaceClass = 0x03;    // HID
    pinterface->bInterfaceSubClass = 0x01; // BIOS
    pinterface->bInterfaceProtocol = 0x01; // 键盘
    pinterface->iInterface = 5;            // 接口名称字符串序号
    ...
    phid->wDescriptorLength0 = REPORTDESCRIPTOR_SIZE_KEYBOARD; //下级描述符长度
    ...
    pendpoint[0].wMaxPacketSize = app.endpoint1->maxPacketSizeIn;
    pendpoint[0].bInterval = 0x0a; // 查询间隔

    pendpoint[1].bLength = sizeof(USB_EndpointDescriptor);
    pendpoint[1].bDescriptorType = ENDPOINT_DESCRIPTOR;
    pendpoint[1].bEndpointAddress = 0x01; // OUT, address 1
    pendpoint[1].bmAttributes = 0x03;     // interrupt
    pendpoint[1].wMaxPacketSize = app.endpoint1->maxPacketSizeOut;
    pendpoint[1].bInterval = 0x0a; // 查询间隔

    pinterface = pendpoint + 2;
    pinterface->bLength = sizeof(USB_InterfaceDescriptor);
    pinterface->bDescriptorType = INTERFACE_DESCRIPTOR;
    ...
    pinterface->iInterface = 6;            // 接口名称字符串序号
    ...
    phid->bNumDescriptors = 1;
    phid->bDescriptorType0 = REPORT_DESCRIPTOR;
    phid->wDescriptorLength0 = REPORTDESCRIPTOR_SIZE_MOUSE; //下级描述符长度

    pendpoint = phid + 1;
    pendpoint[0].bLength = sizeof(USB_EndpointDescriptor);
    pendpoint[0].bDescriptorType = ENDPOINT_DESCRIPTOR;
    pendpoint[0].bEndpointAddress = 0x82; // IN, address 2
    pendpoint[0].bmAttributes = 0x03;     // interrupt
    pendpoint[0].wMaxPacketSize = app.endpoint2->maxPacketSizeIn;
    pendpoint[0].bInterval = 0x0a; // 查询间隔
    ...
    // pendpoint[1].bEndpointAddress = 0x01; // OUT, address 1
    // pendpoint[1].bmAttributes = 0x03;     // interrupt
    // pendpoint[1].wMaxPacketSize = app.endpoint1->maxPacketSize;
    // pendpoint[1].bInterval = 0x0a; // 查询间隔
}
...
void getStringDescriptor(const unsigned short wValue, const unsigned short wIndex)
{
    ...
    if (wIndex == 0x409) // 字符串英文内容
    {
        ...
        case 2:
            DEBUG("string 2\n");
            wcscpy((wchar_t *)pstring->wData, L"Hello Product2!");
            break;
        case 3:
            DEBUG("string 3\n");
        ...
        case 5:
            DEBUG("string 5\n");
            wcscpy((wchar_t *)pstring->wData, L"Hello KEYBOARD Interface!");
            break;
        case 6:
            DEBUG("string 5\n");
            wcscpy((wchar_t *)pstring->wData, L"Hello MOUSE Interface!");
            break;
        default:
            DEBUG("STR_%d\n", wValue & 0xff);
...
int getReportDescriptor(unsigned short wIndex)
{
    ...
    {
    case 0: 
        if(devReq->wLength==0x85)
        {
            printf("interface00");
            memcpy(preport, ReportDescriptor_KEYBOARD, REPORTDESCRIPTOR_SIZE_KEYBOARD);
			memcpy(preport+REPORTDESCRIPTOR_SIZE_KEYBOARD, ReportDescriptor_MOUSE, REPORTDESCRIPTOR_SIZE_MOUSE);
            return REPORTDESCRIPTOR_SIZE_KEYBOARD+REPORTDESCRIPTOR_SIZE_MOUSE;
        }
        printf("interface01");
        memcpy(preport, ReportDescriptor_KEYBOARD, REPORTDESCRIPTOR_SIZE_KEYBOARD);
				//memcpy(preport+REPORTDESCRIPTOR_SIZE_KEYBOARD, ReportDescriptor_MOUSE, REPORTDESCRIPTOR_SIZE_MOUSE);
        return REPORTDESCRIPTOR_SIZE_KEYBOARD;
        break;
    case 1:
        ...
        printf("interface11");
        memcpy(preport, ReportDescriptor_MOUSE, REPORTDESCRIPTOR_SIZE_MOUSE);
        return REPORTDESCRIPTOR_SIZE_MOUSE;
        break;
    default:
        break;
    } 
...
{
    USB_BufferDescriptor[EPx].COUNT_TX = length;
    usbWritePMA(USB_BufferDescriptor[EPx].ADDR_TX, buff, USB_BufferDescriptor[EPx].COUNT_TX);
    SET_STAT_TX(GET_EPxR(EPx), VALID);
    DEBUG("\n\n----1111-----\n\n");
}
static USB_EP0 endPoint0 =
    {
        .maxPacketSizeIn = 0x10,
        .maxPacketSizeOut=0x10,
...
static USB_EP1 endPoint1 =
    {
        .maxPacketSizeIn = 0x10,
        .maxPacketSizeOut=0x10,
        .isAvailable = 1,
};
static USB_EP2 endPoint2 =
    {
        .maxPacketSizeIn = 0x10,
        .maxPacketSizeOut=0x10,
        .isAvailable = 1,
