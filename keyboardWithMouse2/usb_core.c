...
void usbInit()
{
    ...
    //初始化端点0，1
    //对于程序而言buffer起始地址为USB_USB_BufferDescriptor[3];
    //对于usb模块而言buffer起始地址为偏移地址的一半;
    //真实程序偏移4*4*3=48
    int offset = 48;
    USB_BufferDescriptor[0].ADDR_TX = offset;
    USB_BufferDescriptor[0].COUNT_TX = app.endpoint0->maxPacketSizeIn;
    USB_BufferDescriptor[0].ADDR_RX = USB_BufferDescriptor[0].ADDR_TX + USB_BufferDescriptor[0].COUNT_TX;
    USB_BufferDescriptor[0].COUNT_RX = (app.endpoint0->maxPacketSizeOut/2) << 10;

    USB_BufferDescriptor[1].ADDR_TX = USB_BufferDescriptor[0].ADDR_RX + USB_BufferDescriptor[0].COUNT_RX;
    USB_BufferDescriptor[1].COUNT_TX = app.endpoint1->maxPacketSizeIn;
    USB_BufferDescriptor[1].ADDR_RX = USB_BufferDescriptor[1].ADDR_TX + USB_BufferDescriptor[1].COUNT_TX;
    USB_BufferDescriptor[1].COUNT_RX = (app.endpoint1->maxPacketSizeOut/2) << 10;

    USB_BufferDescriptor[2].ADDR_TX = USB_BufferDescriptor[1].ADDR_RX + USB_BufferDescriptor[1].COUNT_RX;
    USB_BufferDescriptor[2].COUNT_TX = app.endpoint2->maxPacketSizeIn;
    USB_BufferDescriptor[2].ADDR_RX = USB_BufferDescriptor[2].ADDR_TX + USB_BufferDescriptor[2].COUNT_TX;
    USB_BufferDescriptor[2].COUNT_RX = (app.endpoint2->maxPacketSizeOut/2) << 10;
...
static void usbStandardGetDescriptor()
{
    ...
    case INTERFACE_RECIPIENT:
        app.dataToSend = app.endpoint0->getReportDescriptor(devReq->wIndex);
        break;
    ...
    usbEPxSend(EP0, app.endpoint0->maxPacketSizeIn);
}
...
static void usbEPxOut(unsigned char epId)
{
    ...
    if(epId==2)
    {
        length = USB_BufferDescriptor[2].COUNT_RX & (0x03ff);
        if (length > 0)
        {
            usbReadPMA(USB_BufferDescriptor[2].ADDR_RX, app.pread, length);
            DUMP_BUFFER(app.pread, length);
        }
        else
        {
...
static void usbEPxIn(unsigned char epId)
{
    ...
    {
        //继续发送未发完数据
        usbEPxSend(EP1, app.endpoint1->maxPacketSizeIn);
        app.endpoint1->isAvailable = 1;
    }
    if (epId == 1)
    {
        //继续发送未发完数据
        usbEPxSend(EP2, app.endpoint2->maxPacketSizeIn);
        app.endpoint2->isAvailable = 1;
    }
}
static void usbTransferHandler()
{
